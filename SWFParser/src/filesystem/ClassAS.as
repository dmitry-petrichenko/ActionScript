package filesystem
{
	...
	public class ClassAS
	{
		...
		private function createClass(path:String):void
		{
			...
			var classContentGenerator:ClassContentGenerator = new ClassContentGenerator(new class1(), path);
			_file = _file.resolvePath(_file.nativePath + "\\" + Utils.getClassNameByPath(path));
			var fileManager:FileManager = new FileManager();
			fileManager.saveFile(_file, classContentGenerator.contentText);
			//Stream.dispatchEvent(StreamEvent.SAVE_FILE, {content: classContentGenerator.contentText, file: _file});
		}
